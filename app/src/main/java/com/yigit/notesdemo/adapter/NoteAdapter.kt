...
class NoteAdapter(private val context: Context,private val noteList: List<Note>) :
    ...
    override fun onBindViewHolder(holder: NoteViewHolder, position: Int) {
        ...
        holder.binding.textViewNoteItem.text = noteList[position].title

        holder.itemView.setOnClickListener {
            val action =
                HomeFragmentDirections.actionHomeFragmentToDetailFragment(id = noteList[position].id)
            Navigation.findNavController(it).navigate(action)
        }

        holder.binding.buttonDeleteItem.setOnClickListener {
            mDisposable.add(
                noteDAO.delete(note = noteList[position]).subscribeOn(Schedulers.io())
